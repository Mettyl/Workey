...
public class NewTaskFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (getActivity() != null) {
            ...
            if (NewTaskFragmentArgs.fromBundle(getArguments()).getEdit()) {
                editedTask = viewModel.getCurrentlyCreatedTask();
                viewModel.delete(editedTask);
            } else {
                resetAllViews();
            }
    ...
    public boolean finishCreatingTask() {
        ...
        if (task.getDeadline().getTime() <= new Date().getTime() + task.getDuration()) {
            Toast.makeText(getContext(), R.string.incorrect_date, Toast.LENGTH_SHORT).show();
            correctlyFilled = false;
        }
    ...
    public void onDestroy() {
        ...
        if (NewTaskFragmentArgs.fromBundle(getArguments()).getEdit() && !edited) {
            viewModel.insert(editedTask);
        }
    }
    ...
    private class FindTimeForTasks extends AsyncTask<Task, Void, Boolean> {
        ...
        private Date findStartForTask(Task task, Date fromTime, Date toTime) {
            ...
            while (iterationTimeCal.getTimeInMillis() <= toTimeCal.getTimeInMillis()) {
                ...
                if (fromTimeCal.get(Calendar.DATE) == iterationTimeCal.get(Calendar.DATE) && fromTimeCal.get(Calendar.DATE)
                        != deadLineCal.get(Calendar.DATE)) {
                    //from current time
